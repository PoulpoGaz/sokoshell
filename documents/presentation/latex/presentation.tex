            ...
            \begin{frame}{Un graphe vu comme un arbre}
                \begin{adjustbox}{max totalsize={\textwidth}{\textheight}, center}
                    \begin{customtree}
                        ...
                        % \node(c2){
                        %     \draw[line width=2mm,red] (sameState2.center)  circle [x radius=1.8cm, y radius=1.5cm];
                        % };
                        % \node(c11){
                        %     \draw[line width=2mm,red] (sameState12.center)  circle [x radius=1.8cm, y radius=1.5cm];
                        % };
                        % \node(c12){
                        %     \draw[line width=2mm,red] (sameState22.center)  circle [x radius=1.8cm, y radius=1.5cm];
                        % };
            ...
            \begin{frame}{Détection des positions mortes \textit{(dead positions)}}
                ...
                \only<1>{
                    \begin{tikzpicture}
                        ...
                        \draw[->, line width = 0.8mm] (before) -- (after);
                    \end{tikzpicture}
                }
                \only<2> {
                    \begin{enumerate}
            ...
            \begin{frame}{Détection de \textit{freeze deadlocks}}
                \begin{center}
                    \begin{tikzpicture}
                        ...
                        \draw[->, visible on=<2-4>] (start.east)  -- (first.west);
                        \draw[->, visible on=<3-4>] (first.south) -- (second.north);
                        \draw[->, visible on=<4-4>] (second.west) -- (third.east);
                    \end{tikzpicture}
                \end{center}
            \end{frame}
