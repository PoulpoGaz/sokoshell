...
public class CommandLine {
    ...
    private Redirect output = NONE;
    private Redirect input = NONE;
        ...
        public OptionSpec findOption(String name) {
            ...
            if (spec == null) {
                if (name.length() == 1) {
                    ...
                    spec = options.values()
                            .stream()
                            .filter((o) -> contains(o.getShortNames(), c))
                            .findFirst()
                            .orElse(null);
                } else {
                    spec = options.values()
                            .stream()
                            .filter((o) -> contains(o.getLongNames(), name))
                            .findFirst()
                            .orElse(null);
                }
            }
    ...
    private static class CommandDescriberImpl implements CommandDescriber {
        ...
        public String getVaArgsDescription() {
            return first(spec.getVaargs().vaArgs.description());
        }
